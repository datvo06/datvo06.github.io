...
function renderGrid(ctx, jsonStr, canvasSize) {
  ...
  // Collect entities into a per-cell map so two objects on the same cell
  // are merged instead of overdrawing (matters for sand-style updates where
  // Autumn's parallel `updateObj` can land two grains on one cell).
  const cells = new Map();
  // Lighter-color names (the "dry" / non-liquid states) should win on tie so
  // wet sand doesn't visually erase a dry pile underneath it.
  const lightnessBias = (col) => {
  ...
  for (const key of Object.keys(parsed)) {
    ...
    for (const entity of entities) {
      ...
      if (!prior || lightnessBias(entity.color) > lightnessBias(prior.color)) {
        cells.set(k, entity);
      }
    }
  }
  ...
  for (const entity of cells.values()) {
    const { x, y } = entity.position;
